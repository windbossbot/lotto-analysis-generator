...
- 기본 추천은 최근성, 번호별 백테스트, 페어 흐름을 합쳐 후보를 만들고, 마지막 5세트 선택에서는 같은 번호가 세트 전체에 2회를 넘기지 않도록 눌러서 5등권 커버리지를 넓히도록 보정합니다.
- 최근 10회 기준 `소수 개수`, `3의 배수 개수`, `직전 회차 겹침`, `최대 간격`, `적당히 쉬던 번호(warm)` 조건도 함께 반영해 과도한 쏠림을 줄입니다.
- `1,2,3,4,5,6` 같은 강한 연속수나 등차수열 성격이 큰 조합은 후보 단계에서 더 약하게 보고, 최근 8회 기준 백테스트로 요약 안정성을 조금 더 높였습니다.
- `운적인 요소` 체크박스를 켜면 실시간 추천과 맞춤 1조합 생성에만 점수 기준 최대 10% 랜덤 가산이 들어갑니다.
- 비교 기준을 흔들지 않기 위해 백테스트 계산에는 `운적인 요소`를 반영하지 않습니다.
- 실제 당첨 확률은 수학적으로 고정이므로, 이 앱은 중복 분산, 구간 균형, 페어 흐름, 홀짝 균형으로 조합 품질과 커버리지를 다듬는 쪽에 초점을 둡니다.
